...
### A8: `voice_journal.py` was a stub, now implemented with graceful degradation

- **Why inferred:** The module previously wrote silent WAV frames as a placeholder.
- **Resolution:** Real audio recording implemented via `sounddevice`/`PyAudio` when a microphone is available. Falls back to text-only journaling when no audio hardware is present.
- **Evidence:** `src/wellness/voice_journal.py`
- **Confidence:** Confirmed

### A9: `store_listing.md` was audited and corrected

- **Why inferred:** Previously advertised "Focus Sessions" / "Pomodoro-style focus timers" before they were implemented.
- **Resolution:** Store listing audited against actual source code; false claims removed, feature descriptions updated. Focus Sessions and Voice Journal are now implemented and accurately described.
- **Evidence:** `windows_store/store_listing.md`
- **Confidence:** Confirmed
