# Paulatim Privacy Policy

_Last updated: 2026-07-14_

_Visible-name correction — 2026-08-28: this policy's substantive data practices
are unchanged; only the visible product name is now Paulatim. Stable local
`ample.*` filenames and older Hearth/Ample migration references are preserved
for compatibility and historical accuracy._

Paulatim is a local-first personal organizer. This policy separates information
the app stores on your device from information the developer receives.

## Your consent

On a new profile, Paulatim asks for express consent before it stores personal
...
in-app erase control. Paulatim records the time of consent in the encrypted
...
For a legacy profile, Paulatim first encrypts the records already stored by the
older app as described under **Migration from older releases**. It then
...
Paulatim data**. Withdrawal deletes all app records and encrypted rollback
snapshots, destroys the key that decrypts them, creates a fresh empty store,
and then returns Paulatim to onboarding.
...
An older Hearth or Ample profile may also contain optional condition-label metadata.
...
Paulatim is a personal organization and reflection tool. It is not a medical
device, healthcare service, diagnosis, treatment, or emergency monitor.

## How local records are protected

Paulatim runs SQLite in memory while the app is open. At rest, it persists only a
...
The database uses a random 256-bit key. Paulatim protects that key with Electron's
...
Paulatim refuses to create or open personal records when secure OS-backed key
...
only in app memory while Paulatim is running. Paulatim best-effort clears its
...
The encrypted files live in Paulatim's operating-system app-data directory under
`data/`:
...
that session. While Paulatim is open, decrypted records and the database key exist
in the app process's memory. The operating system may copy process memory into
swap, hibernation, crash, or diagnostic storage; Paulatim cannot guarantee
...
application-level encryption. On first secure startup, Paulatim:
...
If any verification or write fails, Paulatim leaves the legacy files in place and
does not silently create an empty replacement. During an unfinished migration,
Paulatim can recover from the authenticated rolling backup and then the temporary
...
you used a pre-encryption release under an earlier product name.

## Exports

Paulatim exports only after you choose a destination in an operating-system Save
dialog.
...
These user-requested JSON and PDF files are **not encrypted by Paulatim**. Once an
export is saved, its location, backup, protection, deletion, and any sharing are
under your control. Erasing Paulatim's database does not erase previous exports.

## What the developer receives

The app has no account system, advertising identifiers, analytics, telemetry,
crash reporter, cloud API, or record-sync service. Paulatim does not transmit the
...
policies. Paulatim does not transmit your database contents when opening a link.

## Access, deletion, and recovery limits

Use **Export all data** to access a portable copy of every record stored by
Paulatim. Use **Erase all Paulatim data** to remove all in-app records and
...
credential store or `ample.key` is lost, Paulatim fails closed rather than
...
which require a free GitHub account and sign-in. Never post private Paulatim
