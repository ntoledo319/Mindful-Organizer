...
Hearth is a local-first personal organizer. This policy separates information
the app stores on your device from information the developer receives.

## Your consent

On a new profile, Hearth asks for express consent before it stores personal
...
in-app erase control. Hearth records the time of consent in the encrypted
...
For a legacy profile, Hearth first encrypts the records already stored by the
older app as described under **Migration from older Hearth versions**. It then
...
Hearth data**. Withdrawal deletes all app records and encrypted rollback
snapshots, destroys the key that decrypts them, creates a fresh empty store,
and then returns Hearth to onboarding.
...
An older Hearth profile may also contain optional condition-label metadata.
...
Hearth is a personal organization and reflection tool. It is not a medical
device, healthcare service, diagnosis, treatment, or emergency monitor.

## How local records are protected

Hearth runs SQLite in memory while the app is open. At rest, it persists only a
...
The database uses a random 256-bit key. Hearth protects that key with Electron's
...
Hearth refuses to create or open personal records when secure OS-backed key
...
only in app memory while Hearth is running. Hearth best-effort clears its
...
The encrypted files live in Hearth's operating-system app-data directory under
`data/`:

- `hearth.secure` — current authenticated database snapshot;
- `hearth.secure.backup` — previous authenticated snapshot;
- `hearth.secure.migration-backup` — temporary original snapshot retained only
  until the migrated primary and rolling backup both authenticate; and
- `hearth.key` — the database key encrypted by the operating system.
...
that session. While Hearth is open, decrypted records and the database key exist
in the app process's memory. The operating system may copy process memory into
swap, hibernation, crash, or diagnostic storage; Hearth cannot guarantee
forensic erasure of those system-managed copies. Full-disk encryption reduces
that exposure.

## Migration from older Hearth versions

Older versions stored `hearth.db` and possible SQLite WAL/SHM files without
application-level encryption. On first secure startup, Hearth:
...
If any verification or write fails, Hearth leaves the legacy files in place and
does not silently create an empty replacement. During an unfinished migration,
Hearth can recover from the authenticated rolling backup and then the temporary
...
you used a pre-encryption Hearth version.

## Exports

Hearth exports only after you choose a destination in an operating-system Save
dialog.
...
These user-requested JSON and PDF files are **not encrypted by Hearth**. Once an
export is saved, its location, backup, protection, deletion, and any sharing are
under your control. Erasing Hearth's database does not erase previous exports.
...
policies. Hearth does not transmit your database contents when opening a link.

## Access, deletion, and recovery limits

Use **Export all data** to access a portable copy of every record stored by
Hearth. Use **Erase all Hearth data** to remove all in-app records and
...
credential store or `hearth.key` is lost, Hearth fails closed rather than
