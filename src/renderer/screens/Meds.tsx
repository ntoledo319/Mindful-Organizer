...
import { EmptyState, InlineError, Modal, PageHeader, QueryErrorState, Spinner } from '../components/ui';
...
export function Medications() {
  ...
  const { data: meds, isLoading, isError, refetch } = useQuery({
  ...
  if (isError || !meds) {
    return <QueryErrorState title="Medication references didn't load" onRetry={() => void refetch()} />;
  }
  ...
  return (
    <div>
      <PageHeader
        title="Medications"
        subtitle="A local reference for names, doses, and usual times. Paulatim does not send reminders or monitor adherence."
        action={
      ...
      {(toggleMutation.isError || deleteMutation.isError) && (
        <div className="mb-5">
          <InlineError>Paulatim could not update this reference. Try again.</InlineError>
        </div>
      )}
              ...
              >
                <div className="flex items-center gap-4 flex-1 min-w-0">
                  ...
                  <div className="min-w-0">
                    <h3 className={`truncate font-medium text-base ${m.active ? 'text-text-primary dark:text-night-text' : 'text-text-muted dark:text-night-muted line-through'}`}>
                      {m.name} <span className="ml-2 text-sm font-normal text-text-muted dark:text-night-muted">{m.dosage}</span>
                    </h3>
                ...
                <div className="flex items-center opacity-0 group-hover:opacity-100 transition-opacity focus-within:opacity-100 shrink-0">
                  <motion.button
                    {...hoverTactile}
                    onClick={() => {
                      if (window.confirm(`Delete the reference for “${m.name}”? This cannot be undone.`)) deleteMutation.mutate(m.id);
                    }}
    ...
    <Modal open={open} onClose={onClose} title="Add Medication">
      <div className="space-y-6 pt-2">
        ...
        <p className="text-xs leading-relaxed text-text-muted dark:text-night-muted">
          Paulatim records this time for reference; it does not send medication reminders.
        </p>

        {createMutation.isError && <InlineError>Paulatim could not save this medication reference. Try again.</InlineError>}
